...
static char *port_mapping[5] = {
#if defined(CONFIG_MT7620_ASIC) 
#if defined(CONFIG_SECOND_IF_MT7610)
...
{
	unsigned int	len[2], length = skb->len;
	END_DEVICE*	ei_local = netdev_priv(dev);
#ifdef CONFIG_RAETH_RW_PDMAPTR_FROM_VAR
	unsigned long tx_cpu_owner_idx0 = sysRegRead(TX_CTX_IDX0);
#endif
	struct net_device_stats *stat = &ei_local->stat;
	struct PDMA_txdesc *tx_desc;
	PDMA_TXD_INFO4_T *ti4;
...
#ifdef CONFIG_PSEUDO_SUPPORT
	if (gmac_no == 2 && ei_local->PseudoDev != NULL) {
	...
	len[0] = skb->len;
	daddr[0] = virt_to_phys(skb->data);     /* skb->data is mapped in caller. */
	len[1] = 0;
	daddr[1] = 0;
	nr_frags = skb_shinfo(skb)->nr_frags;
	frag = &skb_shinfo(skb)->frags[0];
	if (unlikely(nr_frags)) {
		len[0] = skb->len - skb->data_len;

		len[1] = frag->size;
		daddr[1] = pci_map_page(NULL, frag->page, frag->page_offset, frag->size, DMA_TO_DEVICE);
	...
	c = 0;
	while (length > 0) {
		tx_desc = &ei_local->tx_ring0[tx_cpu_owner_idx0];
		ti4 = &tx_desc->txd_info4;
		pskb = &ei_local->skb_free[tx_cpu_owner_idx0];
		tx_cpu_owner_idx0 = (tx_cpu_owner_idx0 + 1) & NUM_TX_DESC_MASK;

		while (unlikely(tx_desc->txd_info2.DDONE_bit == 0)) {
			if (printk_ratelimit())
				printk(KERN_ERR
				       "%s: TX DMA is Busy !! TX desc is Empty!\n",
				       dev->name);
			stat->tx_errors++;	/* FIXME */
		}

		/* fill segment 0 */
		length -= len[0];
		last0 = (!length) ? 1 : 0;
		tx_desc->txd_info1.SDP0 = daddr[0];
		tx_desc->txd_info2.SDL0 = len[0];
		tx_desc->txd_info2.LS0_bit = last0;
		tx_desc->txd_info2.DDONE_bit = 0;

		set_tso(ti4, c, skb);
		set_dest_port(ti4, gmac_no, skb);
		set_checksum_offloading(ti4, skb);
		hw_insert_vlan_tag(ti4, skb);

		/* fill segment 1 */
		length -= len[1];
		last1 = (!length && !last0) ? 1 : 0;
		tx_desc->txd_info3.SDP1 = daddr[1];
		tx_desc->txd_info2.SDL1 = len[1];
		tx_desc->txd_info2.LS1_bit = last1;

		ei_local->nr_txd++;
		if (last0 || last1) {
			*pskb = skb;
			tx_desc->txd_info2.BURST_bit = 0;
			break;
		} else {
			*pskb = NULL;
			tx_desc->txd_info2.BURST_bit = 1;
		}

		/* advanced to next two segments */
		c++;
		for (i = 0; i < 2 && nr_frags; ++i, ++frag, --nr_frags) {
			len[i] = frag->size;
			daddr[i] =
			    pci_map_page(NULL, frag->page, frag->page_offset,
	...
	wmb();
	sysRegWrite(TX_CTX_IDX0, tx_cpu_owner_idx0);
	stat->tx_packets++;
...
#endif
#ifdef CONFIG_PSEUDO_SUPPORT
	struct net_device *ndev = ei_local->PseudoDev;
	PSEUDO_ADAPTER *pAd = netdev_priv(ndev);
#else
#ifdef CONFIG_RAETH_SW_FC
	struct net_device *ndev = dev;
#endif
#endif
	struct net_device_stats *stat = &ei_local->stat;
	struct skb_frag_struct *frag = NULL;
	unsigned int txd_cnt = 0, nr_frags, f;
...
#define MIN_PKT_LEN  64
	...
	if (likely(!nr_frags))
		...
		dma_cache_sync(NULL, skb->data, skb->len - skb->data_len, DMA_TO_DEVICE);
...
#else
	/* estimate how many TX descriptors are required. */
	if (likely(!nr_frags)) {
		txd_cnt = TXD_USE_COUNT(skb->len);
	} else {
		txd_cnt = TXD_USE_COUNT(skb->len - skb->data_len);
		for (f = 0, frag = &skb_shinfo(skb)->frags[0]; f < nr_frags; f++, frag++)
			txd_cnt += TXD_USE_COUNT(frag->size);
		txd_cnt = (txd_cnt + 1) >> 1;
	}

	if (likely((NUM_TX_DESC - ei_local->nr_txd) > txd_cnt)) {
		rt2880_eth_send(dev, skb, gmac_no);
		if ((NUM_TX_DESC - ei_local->nr_txd) == 1) {
...
#else  /*!CONFIG_RAETH_QOS */

	if (ei_local->nr_txd) {
		skb_free_idx = ei_local->free_idx;
		while (1) {
			tx_desc = &ei_local->tx_ring0[skb_free_idx];
			pskb = &ei_local->skb_free[skb_free_idx];
			if (!ei_local->nr_txd ||
			    (tx_desc->txd_info2.DDONE_bit == 0) ||
			    (skb_free_idx == sysRegRead(TX_CTX_IDX0))
			   ) {
				break;
			}
			...
			/* Release skb if and only if the TX descriptor include last segment */
			if (*pskb) {
